...
gulp.task( 'collect-static', function( cb ) {
	...
	projectSettings.project_directories.forEach( function( item ) {
		gulp.src( item + '/**' ).pipe(
			gulp.dest( projectSettings.dist + '/' + item + '/' )
		);
	} );
	cb( );
} );
...
gulp.task( 'compile-js-frontend', function( cb ) {
	gulp.src( [
		'js/src/shared/**/*.js',
		'js/src/frontend/base/**/*.js',
		'js/src/frontend/helpers/**/*.js',
		'js/src/frontend/modules/**/*.js',

		'!js/src/frontend/base/wrapper.js', // Exclude the wrapper, since we will use this to wrap the concatenated files
	] )
		.pipe(
			babel( {
				presets: [
		...
		.pipe( wrap( { src: 'js/src/frontend/base/wrapper.js' } ) ) // Wrap the concatenation result in a wrapper
...
gulp.task( 'compile-js-backend', function( cb ) {
	gulp.src( [
		'js/src/shared/**/*.js',
		'js/src/backend/base/**/*.js',
		'js/src/backend/helpers/**/*.js',
		'js/src/backend/modules/**/*.js',

		'!js/src/backend/base/wrapper.js', // Exclude the wrapper, since we will use this to wrap the concatenated files
	] )
		.pipe(
			babel( {
				presets: [
		...
		.pipe( wrap( { src: 'js/src/backend/base/wrapper.js' } ) ) // Wrap the concatenation result in a wrapper
		.pipe( gulp.dest( './' ) ); // Output result in js folder
...
gulp.task(
	...
	gulp.series( 'compile', function( cb ) {
		...
		gulp.watch( 'js/src/backend/**/*.js', gulp.series( 'compile-js-backend' ) );
		gulp.watch(
			'js/src/frontend/**/*.js',
			gulp.series( 'compile-js-frontend' )
		);
		gulp.watch(
			'js/shared/**/*.js',
